...
- Trade catalog (`trade.ts`) — `products`, `trade_agreements`, `agreement_members`, `tariffs`, `trade_barriers`, `tariff_rate_snapshots` (one row per product/country/rate-type, holding the last-observed rate so `detect-tariff-changes.ts` — see [Section 6.12](#612-notifications-notifications) — has something to diff each run against): 1,307 products · 8 agreements · 395 memberships · 79,528 tariffs · 4,000 barriers.
...
- Tariff-rate-change detection went from a documented stub straight to a real, merged implementation from concurrent work on this repo: a teammate's push (landed via `git pull` — auto-merged with zero conflicts, since this session's own most recent commits never touched the file again after trimming it down for lint cleanup) added a `tariff_rate_snapshots` table (one row per product/country/rate-type, `pnpm db:migrate-tariff-history` to create it) that `detect-tariff-changes.ts` now diffs each run's current MFN rate against, notifying every user tracking that product/country pair (`itemType: "opportunity"`) on a real change and recording a fresh baseline either way. All three notification detectors are genuinely live now — nothing left stubbed.
...
- `pnpm db:migrate-tariff-history` — one-time: creates the `tariff_rate_snapshots` table `detect-tariff-changes.ts` needs to diff rate changes against.
