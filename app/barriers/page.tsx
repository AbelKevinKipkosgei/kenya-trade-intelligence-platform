    ...
    <main className="flex min-h-full flex-1 flex-col bg-white dark:bg-zinc-950">
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col px-5 py-7 sm:px-8 sm:py-9">
        <div className="mb-5 flex items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl text-xs leading-5 text-slate-500 dark:text-zinc-400 sm:text-sm">
              Non-tariff barriers, SPS/technical requirements, quotas, and
              licensing issues affecting Kenyan exports, as reported by the
              responsible agency.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center justify-between gap-3 text-xs text-slate-500 dark:text-zinc-400">
          <p>
            Showing{" "}
            <span className="font-semibold text-ktp-navy dark:text-zinc-200">
              {rows.length}
            </span>{" "}
            of {count} matching barriers
          </p>
              ...
              <div className="flex flex-col sm:flex-row">
                  ...
                  <div>
                    ...
                    {b.hsCode && (
                      <a
                        ...
                        className="mt-2 block text-[11px] font-medium text-kenya-green hover:underline"
                      >
                ...
                <div className="min-w-0 flex-1 px-5 py-4 sm:px-6">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${IMPACT_STYLES[b.impactLevel] ?? ""}`}
                    >
                    ...
                    <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold capitalize text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
                      <span className="mr-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-600 align-middle" />
                      {b.status}
                    </span>
                  ...
                  <h2 className="mt-2 text-sm font-semibold leading-5 text-ktp-navy dark:text-zinc-100">
                    {b.description}
                  </h2>
                  {b.hsCode && (
                    <p className="mt-2 inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[10px] text-slate-600 dark:bg-zinc-800 dark:text-zinc-300">
                      {b.productDescription}{" "}
                      {b.sectorName ? ` · ${b.sectorName}` : ""}
                    </p>
                  )}
