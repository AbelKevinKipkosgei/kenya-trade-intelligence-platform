...
export default async function NewsPage({
  ...
  if (country)
    conditions.push(eq(newsArticles.relatedCountryId, Number(country)));
  if (search) {
    conditions.push(
      or(
  ...
  const [rows, [{ count }], [{ total }], [{ sources }], categoryCounts] =
    await Promise.all([
      db
        .select({
          id: newsArticles.id,
      ...
      db
        .select({
          sources: sql<string>`count(distinct ${newsArticles.sourceName})`,
        })
        .from(newsArticles),
      db
        .select({
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col px-5 py-7 sm:px-8 sm:py-9">
        <div className="mb-5 flex items-start gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="mt-2 max-w-3xl text-xs leading-5 text-slate-500 dark:text-zinc-400 sm:text-sm">
              Kenya-related trade, tariff, agreement, and logistics news,
              ingested from live news sources and refreshed every six hours.
            </p>
            <p className="mt-2 text-[11px] font-semibold text-emerald-700 dark:text-emerald-300">
              Updated recently from trusted news sources
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl border border-emerald-100 bg-[#f5fcf9] p-3 dark:border-emerald-900 dark:bg-emerald-950/20 sm:grid-cols-4">
          ...
          <Metric
            ...
            value={countryOptions.length.toLocaleString()}
          />
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
          <section>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-xl font-semibold text-ktp-navy dark:text-zinc-50">
            ...
            <div className="flex flex-col gap-2">
              ...
              {rows.map((a) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm font-semibold leading-5 text-ktp-navy dark:text-zinc-50">
                    {a.title}
                  </p>
                  <p className="mt-1 line-clamp-2 text-xs leading-4 text-slate-500 dark:text-zinc-400">
                    {a.summary}
                  </p>
                </a>
              ))}
          ...
          <aside className="space-y-3">
            <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
              <h2 className="font-display text-lg font-semibold text-ktp-navy dark:text-zinc-50">
                Coverage by topic
              </h2>
                ...
                {categoryCounts.map((item, index) => (
                  ...
                  >
                    <span className="flex items-center gap-2 text-slate-600 dark:text-zinc-300">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-[10px] font-bold text-slate-500 dark:bg-zinc-800">
                        {index + 1}
                      </span>
                      {item.category}
                    </span>
            ...
            <div className="rounded-xl border border-emerald-100 bg-[#f5fcf9] p-4 dark:border-emerald-900 dark:bg-emerald-950/20">
              <p className="text-xs font-semibold text-emerald-800 dark:text-emerald-300">
                News pulse
              </p>
              <p className="mt-1 text-xs leading-4 text-slate-500 dark:text-zinc-400">
                Fresh trade coverage is refreshed every six hours from the
                ingestion pipeline.
              </p>
            </div>
          </aside>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-r border-emerald-100 px-3 last:border-0 dark:border-emerald-900">
      <p className="text-[10px] text-slate-500 dark:text-zinc-400">{label}</p>
      <p className="mt-1 text-lg font-bold leading-none text-ktp-navy dark:text-zinc-100">
        {value}
      </p>
    </div>
  );
